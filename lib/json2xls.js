...
function getByString(object, path) {
    ...
    //     }
    // }
    return (object[path]==undefined)?null:object[path];
}
...
transform.prepareJson = function(json,config) {
    ...
    res.cols = fields.map(function(key,i) {
        ...
        if (/^https?:/i.test((jsonArr[0][key]||'').toString())) {
            return {
                caption: key,
                type: "hyperlink",
                // beforeCellWrite: function(row, cellData){
                //     // return { text: "https://www.google.com/", href: "https://www.google.com/" };
                //     return cellData;
                // }
            };
        }else{
    ...
    res.rows = jsonArr.map(function(row) {
        return fields.map(function(key) {
            ...
            if (/^https?:/i.test((value||'').toString())) {
